	...
	function createJournalArticleNode($doc, $submission) {
		...
		foreach ($authors as $author) {
			...
			if (empty($author->getLocalizedFamilyName(en_US))) {
				...
				$personNameNode->appendChild($node = $doc->createElementNS($deployment->getNamespace(), 'given_name', htmlspecialchars(ucfirst($author->getLocalizedGivenName(en_US)), ENT_COMPAT, 'UTF-8')));
				$personNameNode->appendChild($node = $doc->createElementNS($deployment->getNamespace(), 'surname', htmlspecialchars(ucfirst($author->getLocalizedFamilyName(en_US)), ENT_COMPAT, 'UTF-8')));
			}
